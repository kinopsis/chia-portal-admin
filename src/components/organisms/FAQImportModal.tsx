 ...
 * Modal for importing FAQs from CSV or JSON files
 */
...
import { useFAQExportImport, type ConflictStrategy } from '@/hooks/useFAQExportImport'
import { formatFileSize } from '@/utils/fileUtils'

interface FAQImportModalProps {
  isOpen: boolean
  onClose: () => void
  onImportComplete?: (importedFAQs: any[]) => void
}

export const FAQImportModal: React.FC<FAQImportModalProps> = ({
  isOpen,
  onClose,
  onImportComplete
}) => {
  ...
  const { importFAQs, importLoading, importError, importResult, resetImportResult } = useFAQExportImport()
  ...
  const handleImport = useCallback(async () => {
    ...
    const result = await importFAQs(file, conflictStrategy)
    
    if (result.success) {
      onImportComplete?.(result.faqs || [])
      // Keep modal open to show results
    }
  }, [file, conflictStrategy, importFAQs, onImportComplete])
        ...
        <div>
          ...
          >
            <input
              ...
              className="hidden"
              aria-label="Seleccionar archivo para importar FAQs"
            />
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center">
              ...
              <label htmlFor="strategy-create" className="ml-3 block text-sm text-gray-700">
                Crear nuevas FAQs (puede duplicar)
              </label>
            </div>
          </div>
        </div>
        ...
        {importResult && (
          <div className={`rounded-md p-4 ${
            ...
            <div className="text-sm text-gray-700 space-y-1">
              ...
              {importResult.success && (
                <>
                  <p>Creadas: {importResult.created}</p>
                  <p>Actualizadas: {importResult.updated}</p>
                  <p>Omitidas: {importResult.skipped}</p>
                </>
              )}
